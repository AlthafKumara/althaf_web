...
export default function ActivityDashboard() {
  const { data, isLoading, selectedYear, setSelectedYear } = useActivityController();
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-text-primary font-medium truncate">
                            {commit.message.split('`').map((part: string, i: number) =>
                              i % 2 === 1
                                ? <code key={i} className="text-primary bg-primary/10 px-1 rounded">{part}</code>
                                : part
                            )}
                          </p>
          ...
          <div className="w-full lg:w-48 flex flex-row lg:flex-col gap-2">
            {YEARS.map((year) => (
              <button
                ...
                disabled={isLoading}
                className={`flex-1 lg:flex-none px-6 py-4 rounded-xl text-sm font-bold transition-all duration-300 ${selectedYear === year
