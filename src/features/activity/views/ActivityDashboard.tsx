/**
 * @fileoverview Activity Dashboard View
 *
 * The main view component for the Activity feature. Displays:
 * - A year-selectable contribution heatmap
 * - A feed of the 5 most recent commits
 * - Year filter buttons in a sidebar
 *
 * This is a **Client Component** because it manages local state
 * (selected year, loading status) and triggers a Server Action
 * to fetch data on year change.
 *
 * Data Flow:
 *   User selects year → calls `fetchActivityData` (server action)
 *   → receives `ActivityDataPayload` → renders HeatmapGrid + commit feed
 *
 * @module features/activity/views/ActivityDashboard
 */

"use client";

import { useEffect, useState } from "react";
import { fetchActivityData } from "../controllers/activityAction";
import type { ActivityDataPayload } from "../models/ActivityModel";
...
/** Years available for selection in the sidebar filter. */
const AVAILABLE_YEARS = [2026, 2025, 2024];

/**
 * Renders the complete Activity Dashboard section.
 *
 * Contains:
 * - Header with contribution count
 * - Heatmap grid visualization
 * - Recent commit feed with platform icons
 * - Year filter sidebar
 */
export default function ActivityDashboard() {
  ...
  /** Total contribution count across the entire selected year. */
  const totalContributions =
    data?.heatmap.reduce((acc, curr) => acc + curr.total, 0) || 0;
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-6xl">
        ...
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-text-secondary">
            Recent contributions across GitHub and GitLab.
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="bg-surface border border-neutral-800 rounded-2xl p-6 md:p-8 min-h-[300px] relative">
              <h3 className="text-xl font-bold text-text-primary mb-6 border-b border-neutral-800 pb-4">
              ...
              {loading ? (
                <div className="space-y-4 animate-pulse">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div
                      key={i}
                      className="h-16 w-full bg-neutral-800/50 rounded-xl"
                    />
                  ))}
                </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-2 text-xs text-text-secondary">
                            <span className="font-mono bg-surface px-2 py-0.5 rounded border border-neutral-800">
                              {commit.repo}
                            </span>
                            <span>
                              {format(
